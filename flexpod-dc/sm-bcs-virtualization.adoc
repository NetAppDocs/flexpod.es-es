...
image:sm-bcs-image39.png["Un diagrama que muestra VMware HA y vSphere Metro Storage Cluster en los sitios A y B de FlexPod a través de una estructura de múltiples sitios"]
...
image:sm-bcs-image40.png["Un diagrama que muestra los servidores Cisco UCS B200 M5 y el almacenamiento AFF A250 en los sitios A y B conectados a través de conmutadores Nexus y un ONTAP Mediator en el sitio C"]
...
image:sm-bcs-image41.png["Un diagrama que muestra los conmutadores vSwitch0, vDS0, iSCSI-A e iSCSI-B con seis enlaces ascendentes vNIC"]
...
image:sm-bcs-image42.png["Una captura de pantalla que muestra los grupos de máquinas virtuales y de hosts correspondientes a los hosts y las máquinas virtuales del Sitio A y del Sitio B en vSphere"]
...
image:sm-bcs-image43.png["Una captura de pantalla del cuadro de diálogo Edit VM/Host Rule, en el que se configuran las máquinas virtuales del sitio A para que se ejecuten en los hosts del sitio A"]

image:sm-bcs-image44.png["Una captura de pantalla del cuadro de diálogo Edit VM/Host Rule para las máquinas virtuales y los hosts del Site B con la política de grupo should run on hosts in group policy"]
...
image:sm-bcs-image45.png["Una captura de pantalla de las opciones avanzadas de HA de vSphere con cuatro almacenes de datos de latido y dos direcciones de aislamiento configuradas"]
...
image:sm-bcs-image46.png["Una captura de pantalla de la configuración de los almacenes de datos de latido de alta disponibilidad (HA) de vSphere HA Heartbeat Datastores con cuatro almacenes de datos especificados seleccionados"]

Para obtener prácticas recomendadas adicionales y configuraciones para VMware HA Cluster y VMware vSphere Metro storage cluster, consulta https://docs.vmware.com/en/VMware-vSphere/7.0/com.vmware.vsphere.avail.doc/GUID-5432CA24-14F1-44E3-87FB-61D937831CF6.html["Crear y usar clústeres de vSphere ha"^], https://core.vmware.com/resource/vmware-vsphere-metro-storage-cluster-vmsc["VMware vSphere Metro Storage Cluster (VMSC)"^] y https://docs.netapp.com/us-en/netapp-solutions-virtualization/vmware/vmw-vmsc-with-smas.html["NetApp ONTAP con SnapMirror active sync (antes SM-BC) y VMware vSphere Metro Storage Cluster (vMSC)"^].
