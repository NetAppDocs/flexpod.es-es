...
[role="lead"]
FabricPool requiere ONTAP 9.2 o posterior. Los requisitos adicionales de FabricPool dependen de la versión de ONTAP que se use y del nivel de nube que se conecte. En versiones anteriores a ONTAP 9.8, FabricPool solo se admite en niveles locales SSD. La compatibilidad con agregados HDD se añadió en ONTAP 9.8. Para plataformas AFF de nivel básico que tienen una capacidad fija y relativamente pequeña, como la NetApp AFF C190, FabricPool puede ser muy eficaz para mover datos inactivos al nivel de nube.
...
* AFF de NetApp
* NetApp FAS FabricPool es compatible con todas las plataformas capaces de ejecutar ONTAP 9.2, excepto las siguientes:
+
** FAS8020
...
NOTE: Consulta el  https://hwu.netapp.com/DetailViews/Home.aspx["Hardware Universe de NetApp"^] para obtener la información más reciente sobre las plataformas compatibles con FabricPool.

* NetApp ONTAP Select
+

NOTE: NetApp recomienda usar niveles locales all-SSD FabricPool.
...
** NetApp StorageGRID 10.3 y posteriores
+

NOTE: Glacier Flexible Retrieval y Glacier Deep Archive no son compatibles.
...
image:cloud-fabricpool_image6.png["Un diagrama que muestra dos controladores AFF A300 de NetApp conectando LIFs entre clústeres a AWS S3"]
...
image:cloud-fabricpool_image7.png["Una captura de pantalla de los resultados del analizador del almacén de objetos de la CLI de ONTAP, en la que se muestran la latencia y el rendimiento de las operaciones PUT y GET"]
...
FabricPool licenses (incluida la capacidad adicional para las licencias existentes) se pueden adquirir en incrementos de 1TB.
